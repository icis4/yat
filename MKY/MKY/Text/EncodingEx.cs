...
namespace MKY.Text
{
	...
	public class EncodingEx : EnumEx, IEquatable<EncodingEx>
	{
		...
		static EncodingEx()
		{
			List<EncodingInfoEx> l = new List<EncodingInfoEx>(7); // Preset the required capacity to improve memory management.
		...
		public virtual string Name
		{
			get
			{
				...
				throw (new NotSupportedException(MessageHelper.InvalidExecutionPreamble + "'" + UnderlyingEnum.ToString() + "' is an encoding that is not (yet) supported!" + Environment.NewLine + Environment.NewLine + MessageHelper.SubmitBug));
			}
		}
		...
		{
			List<EncodingEx> a = new List<EncodingEx>(256); // Preset the initial capacity to improve memory management, 256 is a large enough value.
