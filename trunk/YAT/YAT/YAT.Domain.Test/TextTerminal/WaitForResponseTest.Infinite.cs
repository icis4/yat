				...
				try
				{
					...
					using (var terminalB = TerminalFactory.CreateTerminal(settingsB))
					{
						try
						{
							...
							terminalA.SendTextLine(Text); // 2nd line must infinitely be retained again, even in case of pong-pong.
							Utilities.WaitForIsSendingForSomeTime(terminalA);
							...
							terminalA.SendTextLine(Text); // Line must infinitely be retained.
							terminalA.SendTextLine(Text); // Line must infinitely be retained.
							Utilities.WaitForIsSendingForSomeTime(terminalA);
