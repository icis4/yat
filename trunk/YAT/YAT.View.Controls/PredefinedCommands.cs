...
namespace YAT.View.Controls
{
	...
	public partial class PredefinedCommands : UserControl
	{
		...
		{
			this.commandStateUpdateSuspendedCount++; // No need for "lock"/"Interlocked...()" as WinForms is synchronized onto main thread.
		...
		/// </summary>
		private bool PredefinedCommands_Paint_IsFirst { get; set; } = true;
		...
		{
			if (PredefinedCommands_Paint_IsFirst) {
				PredefinedCommands_Paint_IsFirst = false;
		...
		private void AdjustSubpagePanel(PredefinedCommandPageLayout layout)
		{
			...
			try
			{
				...
				switch (layout)
				{
					...
					default:
					{
						throw (new ArgumentOutOfRangeException("layout", layout, MessageHelper.InvalidExecutionPreamble + "'" + layout + "' is a page layout that is not (yet) supported here!" + Environment.NewLine + Environment.NewLine + MessageHelper.SubmitBug));
					}
				}
			}
			finally
